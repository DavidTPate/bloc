(function (Rx, Promise, Hoek, Joi) {
    ...
    internals.fieldSchema = Joi.object().keys({
        ...
        $exists: Joi.boolean(),
        $type: Joi.string(),
        $mod: Joi.array().items(Joi.number()).min(2).max(2),
        ...
        $not: Joi.object().keys({
            ...
            $where: Joi.func(),
            $all: Joi.array().items(Joi.any())
        }).or('$eq', '$gt', '$gte', '$lt', '$lte', '$ne', '$in', '$nin', '$exists', '$type', '$mod', '$regex', '$where', '$all')
    }).or('$eq', '$gt', '$gte', '$lt', '$lte', '$ne', '$in', '$nin', '$exists', '$type', '$mod', '$regex', '$where', '$all', '$not');
