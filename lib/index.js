(function (Rx, Promise, Hoek, Joi) {
    ...
    internals.aggregateSchema = Joi.object().keys({
        $match: internals.filterSchema,
    ...
    internals.aggregate = function aggregate(items, aggregation) {

        return items;
    };

    module.exports = {
        aggregate: function aggregate(items, aggregation, options) {
            ...
            return new Promise(function (resolve, reject) {
                ...
                var validation = Joi.validate(aggregation, internals.aggregateSchema);
                ...
                if (aggregation.$limit) {
                    results = results.take(aggregation.$limit);
                }

                results = internals.filter(results, aggregation.$match || {});
                results = internals.aggregate(results, aggregation);
